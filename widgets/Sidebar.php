...
use bright_tech\yii2theme\adminlte\assets\AdminlteAsset;
...
{
    public $warpClass = 'sidebar-menu';

    public $iconClass = 'menu-icon';

    public $activateParents = true;

    public $defaultDropDownBTagOptions = ['class' => 'pull-right-container'];
...
//        $linkHtml = $this->renderItemLink($item);
        return Html::tag('li', $this->renderItemLink($item) , $options);
    ...
    public function renderDropDownItem($item, &$active = false)
    {
        ...
        return Html::tag('li', $linkHtml . $subItems, $options);
    }
    ...
    protected function renderItemLink($item)
    {
        ...
        $bTagHtml = '';
        if ($items !== null) {
            $bTagIcon = Html::tag('i','',['class'=>'fa fa-angle-left pull-right']);
            $bTagHtml = Html::tag('span', $bTagIcon, $this->defaultDropDownBTagOptions);
